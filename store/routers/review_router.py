...
async def create_review(book_id: str, review: ReviewCreate, db: AsyncIOMotorClient = Depends(get_database)):
    service = ReviewService(db)
...
async def update_review(book_id: str, review_id: str, review: ReviewUpdate, db: AsyncIOMotorClient = Depends(get_database)):
    service = ReviewService(db)
